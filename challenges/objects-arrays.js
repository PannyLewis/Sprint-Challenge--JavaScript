...
let contactInfo = (graduates) => {
  nameEmail = [];
  graduates.forEach((item) =>
    nameEmail.push(item.first_name + " " + item.email)
  );
  return nameEmail;
};
...
const unisWithUni = (graduates) => {
  return graduates.filter((i) => i.university.includes("Uni"));
};
...
let lowCaseAnimalNames = (zooAnimals) => {
...
console.log("populationTotal", populationTotal(zooAnimals));
